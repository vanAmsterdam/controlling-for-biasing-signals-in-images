...
parser.add_argument('--out-dir', default='slices', help='place to put the slices')
parser.add_argument('--in-dir', default='/local_scratch/wamsterd/data/lidc/nodules2d', help='input directory')
parser.add_argument('--min-size', default=20, type=int, help='minimal size of noduls in mm')
parser.add_argument('--splits', default='train,valid,test', help='which spits to create, separate with ,')
...
def main(args):
    ...
    df_ann = pd.read_csv(os.path.join("resources", "annotation_df.csv"))
    ...
    slice_df.to_csv(os.path.join("resources", "slice_df.csv"), index=False)
