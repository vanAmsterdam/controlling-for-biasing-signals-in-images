...
LOG_FILE = "log/lidc-preprocessing.log"
MAX_WORKES = 1
OUT_SIZE = (51,51,51)
...
DO_RESAMP = False # resample and crop
...
if not os.path.exists(os.path.join("resources", "scan_df.csv")):
    scan_df_dict = {}
    ...
    scan_df.to_csv(os.path.join("resources", "scan_df.csv"), index=True, index_label="patient_id")
else:
    scan_df = pd.read_csv(os.path.join("resources", "scan_df.csv"))
...
if not os.path.exists(os.path.join("resources", "annotation_df.feather")):
    ...
    annotation_df.to_csv(os.path.join("resources", "annotation_df.csv"), index=False)
else:
    ...
    for ann_id, ann in tqdm(anns.items()):
        ...
        if current_pid != last_pid:
            try:
                ...
                print(f"loading scan for patient {current_pid}, annotation id {ann_id} failed")
                print(f"loading scan for patient {current_pid}, annotation id {ann_id} failed", file=file)
                continue
            last_pid = current_pid
        ...
        if not ann_id in existing_slices:
            ...
            try:
                ...
                print("cropping failed, skipping...")
                print("cropping failed, skipping...", file=file)
                continue
